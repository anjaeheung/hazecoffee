...
function setMode(m) {
  ...
  if (typeof overlay !== 'undefined' && overlay && m !== 'view') closeOverlay(); // 도구 켜면 겹치기 종료
  mode = m;
...
function openOverlay() {
  ...
  setOverlaySrc(papers.length - 1); // 기본: 마지막 자료 (클리어 파일)
  setHint('그림을 드래그로 움직이고 슬라이더로 크기를 맞춰보세요 · 우클릭 회전');
}
...
function switchPaper(idx, initial = false) {
  if (!initial && overlay) closeOverlay();
